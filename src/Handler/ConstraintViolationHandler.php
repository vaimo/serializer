...
final class ConstraintViolationHandler implements SubscribingHandlerInterface
{
    ...
    {
        return $visitor->serializeArray(iterator_to_array($list), TypeDefinition::fromArray($type), $context);
    }

    public function serializeListToYml(YamlSerializationVisitor $visitor, ConstraintViolationList $list, array $type, SerializationContext $context)
    {
        return $visitor->serializeArray(iterator_to_array($list), TypeDefinition::fromArray($type), $context);
    }
